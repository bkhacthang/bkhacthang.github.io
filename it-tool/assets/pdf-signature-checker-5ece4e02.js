import{d as oA,Z as AB,a as BB,o as z,e as J,i as y,w as m,f as W,t as P,g as u,$ as nB,a0 as EB,V as MB,n as EA,c as QB,W as MA,l as IB,F as YA,r as wB,A as DB,z as gB,Y as CB}from"./index-2bcc7c92.js";import{_ as NB}from"./c-key-value-list.vue_vue_type_script_setup_true_lang-01d7d7fd.js";import{l as _}from"./index-4299a197.js";import{r as GB}from"./___vite-browser-external_commonjs-proxy-da4fd438.js";import{f as VB}from"./convert-ae87c221.js";const TB={flex:"","flex-col":"","gap-2":""},FB={"break-all":"","text-xs":""},rB=oA({__name:"pdf-signature-details",props:{signature:{}},setup(w){const I=w,{signature:C}=AB(I),V={validityPeriod:"Validity period",issuedBy:"Issued by",issuedTo:"Issued to",pemCertificate:"PEM certificate"},T=BB(()=>C.value.meta.certs.map((G,N)=>({...G,validityPeriod:{notBefore:new Date(G.validityPeriod.notBefore).toLocaleString(),notAfter:new Date(G.validityPeriod.notAfter).toLocaleString()},certificateName:`Certificate ${N+1}`})));return(G,N)=>{const Q=NB,t=nB,l=EB;return z(),J("div",TB,[y(l,{data:u(T),headers:V},{validityPeriod:m(({value:F})=>[y(Q,{items:[{label:"Not before",value:F.notBefore},{label:"Not after",value:F.notAfter}]},null,8,["items"])]),issuedBy:m(({value:F})=>[y(Q,{items:[{label:"Common name",value:F.commonName},{label:"Organization name",value:F.organizationName},{label:"Country name",value:F.countryName},{label:"Locality name",value:F.localityName},{label:"Organizational unit name",value:F.organizationalUnitName},{label:"State or province name",value:F.stateOrProvinceName}]},null,8,["items"])]),issuedTo:m(({value:F})=>[y(Q,{items:[{label:"Common name",value:F.commonName},{label:"Organization name",value:F.organizationName},{label:"Country name",value:F.countryName},{label:"Locality name",value:F.localityName},{label:"Organizational unit name",value:F.organizationalUnitName},{label:"State or province name",value:F.stateOrProvinceName}]},null,8,["items"])]),pemCertificate:m(({value:F})=>[y(t,{value:F,label:"View PEM cert"},{value:m(()=>[W("div",FB,P(F),1)]),_:2},1032,["value"])]),_:1},8,["data"])])}}}),dA="TYPE_UNKNOWN",tB="TYPE_INPUT",UB="TYPE_PARSE",cB="TYPE_BYTE_RANGE",YB="VERIFY_SIGNATURE",iB="UNSUPPORTED_SUBFILTER";let xA=class extends Error{constructor(I,C=dA){super(I),this.type=C}};Object.assign(xA,{TYPE_UNKNOWN:dA,TYPE_INPUT:tB,TYPE_PARSE:UB,TYPE_BYTE_RANGE:cB,VERIFY_SIGNATURE:YB,UNSUPPORTED_SUBFILTER:iB});var wA=xA,DA={},$={};$.byteLength=hB;$.toByteArray=aB;$.fromByteArray=xB;var b=[],d=[],RB=typeof Uint8Array<"u"?Uint8Array:Array,QA="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";for(var H=0,lB=QA.length;H<lB;++H)b[H]=QA[H],d[QA.charCodeAt(H)]=H;d["-".charCodeAt(0)]=62;d["_".charCodeAt(0)]=63;function bA(w){var I=w.length;if(I%4>0)throw new Error("Invalid string. Length must be a multiple of 4");var C=w.indexOf("=");C===-1&&(C=I);var V=C===I?0:4-C%4;return[C,V]}function hB(w){var I=bA(w),C=I[0],V=I[1];return(C+V)*3/4-V}function eB(w,I,C){return(I+C)*3/4-C}function aB(w){var I,C=bA(w),V=C[0],T=C[1],G=new RB(eB(w,V,T)),N=0,Q=T>0?V-4:V,t;for(t=0;t<Q;t+=4)I=d[w.charCodeAt(t)]<<18|d[w.charCodeAt(t+1)]<<12|d[w.charCodeAt(t+2)]<<6|d[w.charCodeAt(t+3)],G[N++]=I>>16&255,G[N++]=I>>8&255,G[N++]=I&255;return T===2&&(I=d[w.charCodeAt(t)]<<2|d[w.charCodeAt(t+1)]>>4,G[N++]=I&255),T===1&&(I=d[w.charCodeAt(t)]<<10|d[w.charCodeAt(t+1)]<<4|d[w.charCodeAt(t+2)]>>2,G[N++]=I>>8&255,G[N++]=I&255),G}function oB(w){return b[w>>18&63]+b[w>>12&63]+b[w>>6&63]+b[w&63]}function dB(w,I,C){for(var V,T=[],G=I;G<C;G+=3)V=(w[G]<<16&16711680)+(w[G+1]<<8&65280)+(w[G+2]&255),T.push(oB(V));return T.join("")}function xB(w){for(var I,C=w.length,V=C%3,T=[],G=16383,N=0,Q=C-V;N<Q;N+=G)T.push(dB(w,N,N+G>Q?Q:N+G));return V===1?(I=w[C-1],T.push(b[I>>2]+b[I<<4&63]+"==")):V===2&&(I=(w[C-2]<<8)+w[C-1],T.push(b[I>>10]+b[I>>4&63]+b[I<<2&63]+"=")),T.join("")}var gA={};/*! ieee754. BSD-3-Clause License. Feross Aboukhadijeh <https://feross.org/opensource> */gA.read=function(w,I,C,V,T){var G,N,Q=T*8-V-1,t=(1<<Q)-1,l=t>>1,F=-7,i=C?T-1:0,o=C?-1:1,e=w[I+i];for(i+=o,G=e&(1<<-F)-1,e>>=-F,F+=Q;F>0;G=G*256+w[I+i],i+=o,F-=8);for(N=G&(1<<-F)-1,G>>=-F,F+=V;F>0;N=N*256+w[I+i],i+=o,F-=8);if(G===0)G=1-l;else{if(G===t)return N?NaN:(e?-1:1)*(1/0);N=N+Math.pow(2,V),G=G-l}return(e?-1:1)*N*Math.pow(2,G-V)};gA.write=function(w,I,C,V,T,G){var N,Q,t,l=G*8-T-1,F=(1<<l)-1,i=F>>1,o=T===23?Math.pow(2,-24)-Math.pow(2,-77):0,e=V?0:G-1,j=V?1:-1,L=I<0||I===0&&1/I<0?1:0;for(I=Math.abs(I),isNaN(I)||I===1/0?(Q=isNaN(I)?1:0,N=F):(N=Math.floor(Math.log(I)/Math.LN2),I*(t=Math.pow(2,-N))<1&&(N--,t*=2),N+i>=1?I+=o/t:I+=o*Math.pow(2,1-i),I*t>=2&&(N++,t/=2),N+i>=F?(Q=0,N=F):N+i>=1?(Q=(I*t-1)*Math.pow(2,T),N=N+i):(Q=I*Math.pow(2,i-1)*Math.pow(2,T),N=0));T>=8;w[C+e]=Q&255,e+=j,Q/=256,T-=8);for(N=N<<T|Q,l+=T;l>0;w[C+e]=N&255,e+=j,N/=256,l-=8);w[C+e-j]|=L*128};(function(w){var I=$,C=gA,V=typeof Symbol=="function"&&typeof Symbol.for=="function"?Symbol.for("nodejs.util.inspect.custom"):null;w.Buffer=Q,w.SlowBuffer=CA,w.INSPECT_MAX_BYTES=50;var T=2147483647;w.kMaxLength=T,Q.TYPED_ARRAY_SUPPORT=G(),!Q.TYPED_ARRAY_SUPPORT&&typeof console<"u"&&typeof console.error=="function"&&console.error("This browser lacks typed array (Uint8Array) support which is required by `buffer` v5.x. Use `buffer` v4.x if you require old browser support.");function G(){try{var n=new Uint8Array(1),A={foo:function(){return 42}};return Object.setPrototypeOf(A,Uint8Array.prototype),Object.setPrototypeOf(n,A),n.foo()===42}catch{return!1}}Object.defineProperty(Q.prototype,"parent",{enumerable:!0,get:function(){if(Q.isBuffer(this))return this.buffer}}),Object.defineProperty(Q.prototype,"offset",{enumerable:!0,get:function(){if(Q.isBuffer(this))return this.byteOffset}});function N(n){if(n>T)throw new RangeError('The value "'+n+'" is invalid for option "size"');var A=new Uint8Array(n);return Object.setPrototypeOf(A,Q.prototype),A}function Q(n,A,B){if(typeof n=="number"){if(typeof A=="string")throw new TypeError('The "string" argument must be of type string. Received type number');return i(n)}return t(n,A,B)}Q.poolSize=8192;function t(n,A,B){if(typeof n=="string")return o(n,A);if(ArrayBuffer.isView(n))return e(n);if(n==null)throw new TypeError("The first argument must be one of type string, Buffer, ArrayBuffer, Array, or Array-like Object. Received type "+typeof n);if(s(n,ArrayBuffer)||n&&s(n.buffer,ArrayBuffer)||typeof SharedArrayBuffer<"u"&&(s(n,SharedArrayBuffer)||n&&s(n.buffer,SharedArrayBuffer)))return j(n,A,B);if(typeof n=="number")throw new TypeError('The "value" argument must not be of type number. Received type number');var E=n.valueOf&&n.valueOf();if(E!=null&&E!==n)return Q.from(E,A,B);var M=L(n);if(M)return M;if(typeof Symbol<"u"&&Symbol.toPrimitive!=null&&typeof n[Symbol.toPrimitive]=="function")return Q.from(n[Symbol.toPrimitive]("string"),A,B);throw new TypeError("The first argument must be one of type string, Buffer, ArrayBuffer, Array, or Array-like Object. Received type "+typeof n)}Q.from=function(n,A,B){return t(n,A,B)},Object.setPrototypeOf(Q.prototype,Uint8Array.prototype),Object.setPrototypeOf(Q,Uint8Array);function l(n){if(typeof n!="number")throw new TypeError('"size" argument must be of type number');if(n<0)throw new RangeError('The value "'+n+'" is invalid for option "size"')}function F(n,A,B){return l(n),n<=0?N(n):A!==void 0?typeof B=="string"?N(n).fill(A,B):N(n).fill(A):N(n)}Q.alloc=function(n,A,B){return F(n,A,B)};function i(n){return l(n),N(n<0?0:S(n)|0)}Q.allocUnsafe=function(n){return i(n)},Q.allocUnsafeSlow=function(n){return i(n)};function o(n,A){if((typeof A!="string"||A==="")&&(A="utf8"),!Q.isEncoding(A))throw new TypeError("Unknown encoding: "+A);var B=k(n,A)|0,E=N(B),M=E.write(n,A);return M!==B&&(E=E.slice(0,M)),E}function e(n){for(var A=n.length<0?0:S(n.length)|0,B=N(A),E=0;E<A;E+=1)B[E]=n[E]&255;return B}function j(n,A,B){if(A<0||n.byteLength<A)throw new RangeError('"offset" is outside of buffer bounds');if(n.byteLength<A+(B||0))throw new RangeError('"length" is outside of buffer bounds');var E;return A===void 0&&B===void 0?E=new Uint8Array(n):B===void 0?E=new Uint8Array(n,A):E=new Uint8Array(n,A,B),Object.setPrototypeOf(E,Q.prototype),E}function L(n){if(Q.isBuffer(n)){var A=S(n.length)|0,B=N(A);return B.length===0||n.copy(B,0,0,A),B}if(n.length!==void 0)return typeof n.length!="number"||nA(n.length)?N(0):e(n);if(n.type==="Buffer"&&Array.isArray(n.data))return e(n.data)}function S(n){if(n>=T)throw new RangeError("Attempt to allocate Buffer larger than maximum size: 0x"+T.toString(16)+" bytes");return n|0}function CA(n){return+n!=n&&(n=0),Q.alloc(+n)}Q.isBuffer=function(A){return A!=null&&A._isBuffer===!0&&A!==Q.prototype},Q.compare=function(A,B){if(s(A,Uint8Array)&&(A=Q.from(A,A.offset,A.byteLength)),s(B,Uint8Array)&&(B=Q.from(B,B.offset,B.byteLength)),!Q.isBuffer(A)||!Q.isBuffer(B))throw new TypeError('The "buf1", "buf2" arguments must be one of type Buffer or Uint8Array');if(A===B)return 0;for(var E=A.length,M=B.length,D=0,g=Math.min(E,M);D<g;++D)if(A[D]!==B[D]){E=A[D],M=B[D];break}return E<M?-1:M<E?1:0},Q.isEncoding=function(A){switch(String(A).toLowerCase()){case"hex":case"utf8":case"utf-8":case"ascii":case"latin1":case"binary":case"base64":case"ucs2":case"ucs-2":case"utf16le":case"utf-16le":return!0;default:return!1}},Q.concat=function(A,B){if(!Array.isArray(A))throw new TypeError('"list" argument must be an Array of Buffers');if(A.length===0)return Q.alloc(0);var E;if(B===void 0)for(B=0,E=0;E<A.length;++E)B+=A[E].length;var M=Q.allocUnsafe(B),D=0;for(E=0;E<A.length;++E){var g=A[E];if(s(g,Uint8Array)&&(g=Q.from(g)),!Q.isBuffer(g))throw new TypeError('"list" argument must be an Array of Buffers');g.copy(M,D),D+=g.length}return M};function k(n,A){if(Q.isBuffer(n))return n.length;if(ArrayBuffer.isView(n)||s(n,ArrayBuffer))return n.byteLength;if(typeof n!="string")throw new TypeError('The "string" argument must be one of type string, Buffer, or ArrayBuffer. Received type '+typeof n);var B=n.length,E=arguments.length>2&&arguments[2]===!0;if(!E&&B===0)return 0;for(var M=!1;;)switch(A){case"ascii":case"latin1":case"binary":return B;case"utf8":case"utf-8":return BA(n).length;case"ucs2":case"ucs-2":case"utf16le":case"utf-16le":return B*2;case"hex":return B>>>1;case"base64":return tA(n).length;default:if(M)return E?-1:BA(n).length;A=(""+A).toLowerCase(),M=!0}}Q.byteLength=k;function X(n,A,B){var E=!1;if((A===void 0||A<0)&&(A=0),A>this.length||((B===void 0||B>this.length)&&(B=this.length),B<=0)||(B>>>=0,A>>>=0,B<=A))return"";for(n||(n="utf8");;)switch(n){case"hex":return LA(this,A,B);case"utf8":case"utf-8":return GA(this,A,B);case"ascii":return vA(this,A,B);case"latin1":case"binary":return fA(this,A,B);case"base64":return kA(this,A,B);case"ucs2":case"ucs-2":case"utf16le":case"utf-16le":return KA(this,A,B);default:if(E)throw new TypeError("Unknown encoding: "+n);n=(n+"").toLowerCase(),E=!0}}Q.prototype._isBuffer=!0;function Z(n,A,B){var E=n[A];n[A]=n[B],n[B]=E}Q.prototype.swap16=function(){var A=this.length;if(A%2!==0)throw new RangeError("Buffer size must be a multiple of 16-bits");for(var B=0;B<A;B+=2)Z(this,B,B+1);return this},Q.prototype.swap32=function(){var A=this.length;if(A%4!==0)throw new RangeError("Buffer size must be a multiple of 32-bits");for(var B=0;B<A;B+=4)Z(this,B,B+3),Z(this,B+1,B+2);return this},Q.prototype.swap64=function(){var A=this.length;if(A%8!==0)throw new RangeError("Buffer size must be a multiple of 64-bits");for(var B=0;B<A;B+=8)Z(this,B,B+7),Z(this,B+1,B+6),Z(this,B+2,B+5),Z(this,B+3,B+4);return this},Q.prototype.toString=function(){var A=this.length;return A===0?"":arguments.length===0?GA(this,0,A):X.apply(this,arguments)},Q.prototype.toLocaleString=Q.prototype.toString,Q.prototype.equals=function(A){if(!Q.isBuffer(A))throw new TypeError("Argument must be a Buffer");return this===A?!0:Q.compare(this,A)===0},Q.prototype.inspect=function(){var A="",B=w.INSPECT_MAX_BYTES;return A=this.toString("hex",0,B).replace(/(.{2})/g,"$1 ").trim(),this.length>B&&(A+=" ... "),"<Buffer "+A+">"},V&&(Q.prototype[V]=Q.prototype.inspect),Q.prototype.compare=function(A,B,E,M,D){if(s(A,Uint8Array)&&(A=Q.from(A,A.offset,A.byteLength)),!Q.isBuffer(A))throw new TypeError('The "target" argument must be one of type Buffer or Uint8Array. Received type '+typeof A);if(B===void 0&&(B=0),E===void 0&&(E=A?A.length:0),M===void 0&&(M=0),D===void 0&&(D=this.length),B<0||E>A.length||M<0||D>this.length)throw new RangeError("out of range index");if(M>=D&&B>=E)return 0;if(M>=D)return-1;if(B>=E)return 1;if(B>>>=0,E>>>=0,M>>>=0,D>>>=0,this===A)return 0;for(var g=D-M,r=E-B,U=Math.min(g,r),c=this.slice(M,D),h=A.slice(B,E),Y=0;Y<U;++Y)if(c[Y]!==h[Y]){g=c[Y],r=h[Y];break}return g<r?-1:r<g?1:0};function v(n,A,B,E,M){if(n.length===0)return-1;if(typeof B=="string"?(E=B,B=0):B>2147483647?B=2147483647:B<-2147483648&&(B=-2147483648),B=+B,nA(B)&&(B=M?0:n.length-1),B<0&&(B=n.length+B),B>=n.length){if(M)return-1;B=n.length-1}else if(B<0)if(M)B=0;else return-1;if(typeof A=="string"&&(A=Q.from(A,E)),Q.isBuffer(A))return A.length===0?-1:f(n,A,B,E,M);if(typeof A=="number")return A=A&255,typeof Uint8Array.prototype.indexOf=="function"?M?Uint8Array.prototype.indexOf.call(n,A,B):Uint8Array.prototype.lastIndexOf.call(n,A,B):f(n,[A],B,E,M);throw new TypeError("val must be string, number or Buffer")}function f(n,A,B,E,M){var D=1,g=n.length,r=A.length;if(E!==void 0&&(E=String(E).toLowerCase(),E==="ucs2"||E==="ucs-2"||E==="utf16le"||E==="utf-16le")){if(n.length<2||A.length<2)return-1;D=2,g/=2,r/=2,B/=2}function U(UA,cA){return D===1?UA[cA]:UA.readUInt16BE(cA*D)}var c;if(M){var h=-1;for(c=B;c<g;c++)if(U(n,c)===U(A,h===-1?0:c-h)){if(h===-1&&(h=c),c-h+1===r)return h*D}else h!==-1&&(c-=c-h),h=-1}else for(B+r>g&&(B=g-r),c=B;c>=0;c--){for(var Y=!0,O=0;O<r;O++)if(U(n,c+O)!==U(A,O)){Y=!1;break}if(Y)return c}return-1}Q.prototype.includes=function(A,B,E){return this.indexOf(A,B,E)!==-1},Q.prototype.indexOf=function(A,B,E){return v(this,A,B,E,!0)},Q.prototype.lastIndexOf=function(A,B,E){return v(this,A,B,E,!1)};function AA(n,A,B,E){B=Number(B)||0;var M=n.length-B;E?(E=Number(E),E>M&&(E=M)):E=M;var D=A.length;E>D/2&&(E=D/2);for(var g=0;g<E;++g){var r=parseInt(A.substr(g*2,2),16);if(nA(r))return g;n[B+g]=r}return g}function JA(n,A,B,E){return K(BA(A,n.length-B),n,B,E)}function NA(n,A,B,E){return K(PA(A),n,B,E)}function WA(n,A,B,E){return NA(n,A,B,E)}function pA(n,A,B,E){return K(tA(A),n,B,E)}function SA(n,A,B,E){return K(_A(A,n.length-B),n,B,E)}Q.prototype.write=function(A,B,E,M){if(B===void 0)M="utf8",E=this.length,B=0;else if(E===void 0&&typeof B=="string")M=B,E=this.length,B=0;else if(isFinite(B))B=B>>>0,isFinite(E)?(E=E>>>0,M===void 0&&(M="utf8")):(M=E,E=void 0);else throw new Error("Buffer.write(string, encoding, offset[, length]) is no longer supported");var D=this.length-B;if((E===void 0||E>D)&&(E=D),A.length>0&&(E<0||B<0)||B>this.length)throw new RangeError("Attempt to write outside buffer bounds");M||(M="utf8");for(var g=!1;;)switch(M){case"hex":return AA(this,A,B,E);case"utf8":case"utf-8":return JA(this,A,B,E);case"ascii":return NA(this,A,B,E);case"latin1":case"binary":return WA(this,A,B,E);case"base64":return pA(this,A,B,E);case"ucs2":case"ucs-2":case"utf16le":case"utf-16le":return SA(this,A,B,E);default:if(g)throw new TypeError("Unknown encoding: "+M);M=(""+M).toLowerCase(),g=!0}},Q.prototype.toJSON=function(){return{type:"Buffer",data:Array.prototype.slice.call(this._arr||this,0)}};function kA(n,A,B){return A===0&&B===n.length?I.fromByteArray(n):I.fromByteArray(n.slice(A,B))}function GA(n,A,B){B=Math.min(n.length,B);for(var E=[],M=A;M<B;){var D=n[M],g=null,r=D>239?4:D>223?3:D>191?2:1;if(M+r<=B){var U,c,h,Y;switch(r){case 1:D<128&&(g=D);break;case 2:U=n[M+1],(U&192)===128&&(Y=(D&31)<<6|U&63,Y>127&&(g=Y));break;case 3:U=n[M+1],c=n[M+2],(U&192)===128&&(c&192)===128&&(Y=(D&15)<<12|(U&63)<<6|c&63,Y>2047&&(Y<55296||Y>57343)&&(g=Y));break;case 4:U=n[M+1],c=n[M+2],h=n[M+3],(U&192)===128&&(c&192)===128&&(h&192)===128&&(Y=(D&15)<<18|(U&63)<<12|(c&63)<<6|h&63,Y>65535&&Y<1114112&&(g=Y))}}g===null?(g=65533,r=1):g>65535&&(g-=65536,E.push(g>>>10&1023|55296),g=56320|g&1023),E.push(g),M+=r}return XA(E)}var VA=4096;function XA(n){var A=n.length;if(A<=VA)return String.fromCharCode.apply(String,n);for(var B="",E=0;E<A;)B+=String.fromCharCode.apply(String,n.slice(E,E+=VA));return B}function vA(n,A,B){var E="";B=Math.min(n.length,B);for(var M=A;M<B;++M)E+=String.fromCharCode(n[M]&127);return E}function fA(n,A,B){var E="";B=Math.min(n.length,B);for(var M=A;M<B;++M)E+=String.fromCharCode(n[M]);return E}function LA(n,A,B){var E=n.length;(!A||A<0)&&(A=0),(!B||B<0||B>E)&&(B=E);for(var M="",D=A;D<B;++D)M+=$A[n[D]];return M}function KA(n,A,B){for(var E=n.slice(A,B),M="",D=0;D<E.length;D+=2)M+=String.fromCharCode(E[D]+E[D+1]*256);return M}Q.prototype.slice=function(A,B){var E=this.length;A=~~A,B=B===void 0?E:~~B,A<0?(A+=E,A<0&&(A=0)):A>E&&(A=E),B<0?(B+=E,B<0&&(B=0)):B>E&&(B=E),B<A&&(B=A);var M=this.subarray(A,B);return Object.setPrototypeOf(M,Q.prototype),M};function R(n,A,B){if(n%1!==0||n<0)throw new RangeError("offset is not uint");if(n+A>B)throw new RangeError("Trying to access beyond buffer length")}Q.prototype.readUIntLE=function(A,B,E){A=A>>>0,B=B>>>0,E||R(A,B,this.length);for(var M=this[A],D=1,g=0;++g<B&&(D*=256);)M+=this[A+g]*D;return M},Q.prototype.readUIntBE=function(A,B,E){A=A>>>0,B=B>>>0,E||R(A,B,this.length);for(var M=this[A+--B],D=1;B>0&&(D*=256);)M+=this[A+--B]*D;return M},Q.prototype.readUInt8=function(A,B){return A=A>>>0,B||R(A,1,this.length),this[A]},Q.prototype.readUInt16LE=function(A,B){return A=A>>>0,B||R(A,2,this.length),this[A]|this[A+1]<<8},Q.prototype.readUInt16BE=function(A,B){return A=A>>>0,B||R(A,2,this.length),this[A]<<8|this[A+1]},Q.prototype.readUInt32LE=function(A,B){return A=A>>>0,B||R(A,4,this.length),(this[A]|this[A+1]<<8|this[A+2]<<16)+this[A+3]*16777216},Q.prototype.readUInt32BE=function(A,B){return A=A>>>0,B||R(A,4,this.length),this[A]*16777216+(this[A+1]<<16|this[A+2]<<8|this[A+3])},Q.prototype.readIntLE=function(A,B,E){A=A>>>0,B=B>>>0,E||R(A,B,this.length);for(var M=this[A],D=1,g=0;++g<B&&(D*=256);)M+=this[A+g]*D;return D*=128,M>=D&&(M-=Math.pow(2,8*B)),M},Q.prototype.readIntBE=function(A,B,E){A=A>>>0,B=B>>>0,E||R(A,B,this.length);for(var M=B,D=1,g=this[A+--M];M>0&&(D*=256);)g+=this[A+--M]*D;return D*=128,g>=D&&(g-=Math.pow(2,8*B)),g},Q.prototype.readInt8=function(A,B){return A=A>>>0,B||R(A,1,this.length),this[A]&128?(255-this[A]+1)*-1:this[A]},Q.prototype.readInt16LE=function(A,B){A=A>>>0,B||R(A,2,this.length);var E=this[A]|this[A+1]<<8;return E&32768?E|4294901760:E},Q.prototype.readInt16BE=function(A,B){A=A>>>0,B||R(A,2,this.length);var E=this[A+1]|this[A]<<8;return E&32768?E|4294901760:E},Q.prototype.readInt32LE=function(A,B){return A=A>>>0,B||R(A,4,this.length),this[A]|this[A+1]<<8|this[A+2]<<16|this[A+3]<<24},Q.prototype.readInt32BE=function(A,B){return A=A>>>0,B||R(A,4,this.length),this[A]<<24|this[A+1]<<16|this[A+2]<<8|this[A+3]},Q.prototype.readFloatLE=function(A,B){return A=A>>>0,B||R(A,4,this.length),C.read(this,A,!0,23,4)},Q.prototype.readFloatBE=function(A,B){return A=A>>>0,B||R(A,4,this.length),C.read(this,A,!1,23,4)},Q.prototype.readDoubleLE=function(A,B){return A=A>>>0,B||R(A,8,this.length),C.read(this,A,!0,52,8)},Q.prototype.readDoubleBE=function(A,B){return A=A>>>0,B||R(A,8,this.length),C.read(this,A,!1,52,8)};function a(n,A,B,E,M,D){if(!Q.isBuffer(n))throw new TypeError('"buffer" argument must be a Buffer instance');if(A>M||A<D)throw new RangeError('"value" argument is out of bounds');if(B+E>n.length)throw new RangeError("Index out of range")}Q.prototype.writeUIntLE=function(A,B,E,M){if(A=+A,B=B>>>0,E=E>>>0,!M){var D=Math.pow(2,8*E)-1;a(this,A,B,E,D,0)}var g=1,r=0;for(this[B]=A&255;++r<E&&(g*=256);)this[B+r]=A/g&255;return B+E},Q.prototype.writeUIntBE=function(A,B,E,M){if(A=+A,B=B>>>0,E=E>>>0,!M){var D=Math.pow(2,8*E)-1;a(this,A,B,E,D,0)}var g=E-1,r=1;for(this[B+g]=A&255;--g>=0&&(r*=256);)this[B+g]=A/r&255;return B+E},Q.prototype.writeUInt8=function(A,B,E){return A=+A,B=B>>>0,E||a(this,A,B,1,255,0),this[B]=A&255,B+1},Q.prototype.writeUInt16LE=function(A,B,E){return A=+A,B=B>>>0,E||a(this,A,B,2,65535,0),this[B]=A&255,this[B+1]=A>>>8,B+2},Q.prototype.writeUInt16BE=function(A,B,E){return A=+A,B=B>>>0,E||a(this,A,B,2,65535,0),this[B]=A>>>8,this[B+1]=A&255,B+2},Q.prototype.writeUInt32LE=function(A,B,E){return A=+A,B=B>>>0,E||a(this,A,B,4,4294967295,0),this[B+3]=A>>>24,this[B+2]=A>>>16,this[B+1]=A>>>8,this[B]=A&255,B+4},Q.prototype.writeUInt32BE=function(A,B,E){return A=+A,B=B>>>0,E||a(this,A,B,4,4294967295,0),this[B]=A>>>24,this[B+1]=A>>>16,this[B+2]=A>>>8,this[B+3]=A&255,B+4},Q.prototype.writeIntLE=function(A,B,E,M){if(A=+A,B=B>>>0,!M){var D=Math.pow(2,8*E-1);a(this,A,B,E,D-1,-D)}var g=0,r=1,U=0;for(this[B]=A&255;++g<E&&(r*=256);)A<0&&U===0&&this[B+g-1]!==0&&(U=1),this[B+g]=(A/r>>0)-U&255;return B+E},Q.prototype.writeIntBE=function(A,B,E,M){if(A=+A,B=B>>>0,!M){var D=Math.pow(2,8*E-1);a(this,A,B,E,D-1,-D)}var g=E-1,r=1,U=0;for(this[B+g]=A&255;--g>=0&&(r*=256);)A<0&&U===0&&this[B+g+1]!==0&&(U=1),this[B+g]=(A/r>>0)-U&255;return B+E},Q.prototype.writeInt8=function(A,B,E){return A=+A,B=B>>>0,E||a(this,A,B,1,127,-128),A<0&&(A=255+A+1),this[B]=A&255,B+1},Q.prototype.writeInt16LE=function(A,B,E){return A=+A,B=B>>>0,E||a(this,A,B,2,32767,-32768),this[B]=A&255,this[B+1]=A>>>8,B+2},Q.prototype.writeInt16BE=function(A,B,E){return A=+A,B=B>>>0,E||a(this,A,B,2,32767,-32768),this[B]=A>>>8,this[B+1]=A&255,B+2},Q.prototype.writeInt32LE=function(A,B,E){return A=+A,B=B>>>0,E||a(this,A,B,4,2147483647,-2147483648),this[B]=A&255,this[B+1]=A>>>8,this[B+2]=A>>>16,this[B+3]=A>>>24,B+4},Q.prototype.writeInt32BE=function(A,B,E){return A=+A,B=B>>>0,E||a(this,A,B,4,2147483647,-2147483648),A<0&&(A=4294967295+A+1),this[B]=A>>>24,this[B+1]=A>>>16,this[B+2]=A>>>8,this[B+3]=A&255,B+4};function TA(n,A,B,E,M,D){if(B+E>n.length)throw new RangeError("Index out of range");if(B<0)throw new RangeError("Index out of range")}function FA(n,A,B,E,M){return A=+A,B=B>>>0,M||TA(n,A,B,4),C.write(n,A,B,E,23,4),B+4}Q.prototype.writeFloatLE=function(A,B,E){return FA(this,A,B,!0,E)},Q.prototype.writeFloatBE=function(A,B,E){return FA(this,A,B,!1,E)};function rA(n,A,B,E,M){return A=+A,B=B>>>0,M||TA(n,A,B,8),C.write(n,A,B,E,52,8),B+8}Q.prototype.writeDoubleLE=function(A,B,E){return rA(this,A,B,!0,E)},Q.prototype.writeDoubleBE=function(A,B,E){return rA(this,A,B,!1,E)},Q.prototype.copy=function(A,B,E,M){if(!Q.isBuffer(A))throw new TypeError("argument should be a Buffer");if(E||(E=0),!M&&M!==0&&(M=this.length),B>=A.length&&(B=A.length),B||(B=0),M>0&&M<E&&(M=E),M===E||A.length===0||this.length===0)return 0;if(B<0)throw new RangeError("targetStart out of bounds");if(E<0||E>=this.length)throw new RangeError("Index out of range");if(M<0)throw new RangeError("sourceEnd out of bounds");M>this.length&&(M=this.length),A.length-B<M-E&&(M=A.length-B+E);var D=M-E;if(this===A&&typeof Uint8Array.prototype.copyWithin=="function")this.copyWithin(B,E,M);else if(this===A&&E<B&&B<M)for(var g=D-1;g>=0;--g)A[g+B]=this[g+E];else Uint8Array.prototype.set.call(A,this.subarray(E,M),B);return D},Q.prototype.fill=function(A,B,E,M){if(typeof A=="string"){if(typeof B=="string"?(M=B,B=0,E=this.length):typeof E=="string"&&(M=E,E=this.length),M!==void 0&&typeof M!="string")throw new TypeError("encoding must be a string");if(typeof M=="string"&&!Q.isEncoding(M))throw new TypeError("Unknown encoding: "+M);if(A.length===1){var D=A.charCodeAt(0);(M==="utf8"&&D<128||M==="latin1")&&(A=D)}}else typeof A=="number"?A=A&255:typeof A=="boolean"&&(A=Number(A));if(B<0||this.length<B||this.length<E)throw new RangeError("Out of range index");if(E<=B)return this;B=B>>>0,E=E===void 0?this.length:E>>>0,A||(A=0);var g;if(typeof A=="number")for(g=B;g<E;++g)this[g]=A;else{var r=Q.isBuffer(A)?A:Q.from(A,M),U=r.length;if(U===0)throw new TypeError('The value "'+A+'" is invalid for argument "value"');for(g=0;g<E-B;++g)this[g+B]=r[g%U]}return this};var OA=/[^+/0-9A-Za-z-_]/g;function qA(n){if(n=n.split("=")[0],n=n.trim().replace(OA,""),n.length<2)return"";for(;n.length%4!==0;)n=n+"=";return n}function BA(n,A){A=A||1/0;for(var B,E=n.length,M=null,D=[],g=0;g<E;++g){if(B=n.charCodeAt(g),B>55295&&B<57344){if(!M){if(B>56319){(A-=3)>-1&&D.push(239,191,189);continue}else if(g+1===E){(A-=3)>-1&&D.push(239,191,189);continue}M=B;continue}if(B<56320){(A-=3)>-1&&D.push(239,191,189),M=B;continue}B=(M-55296<<10|B-56320)+65536}else M&&(A-=3)>-1&&D.push(239,191,189);if(M=null,B<128){if((A-=1)<0)break;D.push(B)}else if(B<2048){if((A-=2)<0)break;D.push(B>>6|192,B&63|128)}else if(B<65536){if((A-=3)<0)break;D.push(B>>12|224,B>>6&63|128,B&63|128)}else if(B<1114112){if((A-=4)<0)break;D.push(B>>18|240,B>>12&63|128,B>>6&63|128,B&63|128)}else throw new Error("Invalid code point")}return D}function PA(n){for(var A=[],B=0;B<n.length;++B)A.push(n.charCodeAt(B)&255);return A}function _A(n,A){for(var B,E,M,D=[],g=0;g<n.length&&!((A-=2)<0);++g)B=n.charCodeAt(g),E=B>>8,M=B%256,D.push(M),D.push(E);return D}function tA(n){return I.toByteArray(qA(n))}function K(n,A,B,E){for(var M=0;M<E&&!(M+B>=A.length||M>=n.length);++M)A[M+B]=n[M];return M}function s(n,A){return n instanceof A||n!=null&&n.constructor!=null&&n.constructor.name!=null&&n.constructor.name===A.name}function nA(n){return n!==n}var $A=function(){for(var n="0123456789abcdef",A=new Array(256),B=0;B<16;++B)for(var E=B*16,M=0;M<16;++M)A[E+M]=n[B]+n[M];return A}()})(DA);const iA=_,{Buffer:IA}=DA,p=wA,bB=w=>{try{return IA.isBuffer(w)?w:IA.from(w)}catch{throw new p("PDF expected as Buffer.",p.TYPE_INPUT)}},ZB=w=>{const I=w.toString().match(/\/SubFilter\s*\/([\w.]*)/),C=Array.isArray(I)&&I[1];if(!C)throw new p("cannot find subfilter",p.TYPE_PARSE);if(!["adbe.pkcs7.detached","etsi.cades.detached"].includes(C.trim().toLowerCase()))throw new p(`subFilter ${C} not supported`,p.UNSUPPORTED_SUBFILTER)},sB=w=>{const I=iA.asn1.fromDer(w,{parseAllBytes:!1});return iA.pkcs7.messageFromAsn1(I)},q=w=>I=>{const C=new RegExp(`/${w}\\s*\\(([\\w.\\s@,]*)`,"g"),V=[...I.matchAll(C)];return V.length?V[V.length-1][1]:null},uB=w=>{const I=IA.isBuffer(w)?w.toString():w;return{reason:q("Reason")(I),contactInfo:q("ContactInfo")(I),location:q("Location")(I),name:q("Name")(I)}};var ZA={checkForSubFilter:ZB,getSignatureMeta:uB,getMessageFromSignature:sB,preparePDF:bB};const{Buffer:RA}=DA,lA=wA,{getSignatureMeta:jB,preparePDF:yB}=ZA,zB="**********",mB=w=>{const I=w.toString().match(/\/ByteRange\s*\[{1}\s*(?:(?:\d*|\/\*{10})\s+){3}(?:\d+|\/\*{10}){1}\s*\]{1}/g);if(!I)throw new lA("Failed to locate ByteRange.",lA.TYPE_PARSE);const C=I.find(G=>G.includes(`/${zB}`)),T=I.map(G=>G.match(/[^[\s]*(?:\d|\/\*{10})/g)).map(G=>G.map(Number));return{byteRangePlaceholder:C,byteRanges:T}},HB=w=>{const I=yB(w),{byteRanges:C}=mB(I),V=[],T=[];C.forEach(N=>{T.push(RA.concat([I.slice(N[0],N[0]+N[1]),I.slice(N[2],N[2]+N[3])]));const Q=I.slice(N[0]+N[1]+1,N[2]).toString("latin1");V.push(RA.from(Q,"hex").toString("latin1"))});const G=T.map(N=>jB(N));return{byteRanges:C,signatureStr:V,signedData:T,signatureMeta:G}};var JB=HB;const WB=w=>I=>w!==I&&I.issued(w),pB=w=>I=>w.find(WB(I)),SB=w=>I=>!w(I),kB=w=>SB(pB(w)),XB=w=>w.findIndex(kB(w)),vB=w=>I=>w!==I&&w.issued(I),fB=w=>I=>w.findIndex(vB(I)),sA=w=>{const I=Array.from(w),C=XB(I),V=I.splice(C,1);for(;I.length;){const T=V[0],G=fB(I)(T);if(G===-1)I.splice(G,1);else{const[N]=I.splice(G,1);V.unshift(N)}}return V},LB=w=>sA(w)[0];var KB={sortCertificateChain:sA,getClientCertificate:LB};const OB=[`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

R9I4LtD+gdwyah617jzV/OeBHRnDJELqYzmp
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

j2A781q0tHuu2guQOHXvgR1m0vdXcDazv/wor3ElhVsT/h5/WrQ8
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

akhT2+zNVVXxxvjpoixMptEmX36vWkzaH6byHCx+rgIW0lbQL1dTR+iS
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

ZdWCEOrCMc0u
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

V3bqxbYoOb8VZRzI9neWagqNdwvYkQsEjgfbKbYK7p2CNTUQ
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

W8mw0FfB+j564ZfJ
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

5Aq7KkzrCWA5zspi2C5u
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

ThaA63p4ZUWiABqvDA1VZDRIuJK58bRQKfJPIx/abKwfROHdI3hRW8cW
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

NmdspZS11My5vWo1ViHe2MPr+8ukYEywVaCge1ey
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

Ecgh4LMRkWXbtKaIOM5V
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

aZVDADlN9u6wWk5JRFRYX0KD
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

9vuD4DPTCF+hnMJbn0bWtsuRBmOiBuczrD6ogRLQy7rQkgu2npaqBA+K
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

LXpUq3DDfSJlgnCW
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

WD9f
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

8zsxkVrctQgVrXYlCg17LofiDKYGvCYQbTed7N14jHyAxfDZd0jQ
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

SC34MCN32EZLeW32jO06f2ARePTpm67VVMB0gNELQp/B
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

NnReJaH1ZgUufzkVqSr7UIuOhWn0
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

/Wi3MOxw+3WKt21gZ7IeyLnp2KhvAotnDU0mV3HaIPzBSlCNsSi6
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

SjnRBUkLp7Y3gaVdjKozXoEofKd9J+sAro03
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

JuVvw9y4AyHqnxbxLFS1
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

QCj8KWefshNPZiTEUxnpHikV7+ZtsH8tZ/3zbBt1RqPlShfppNcL
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

D0tVNEYqi4Y7
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

KrcYPqcZ2Qt9sTdBQrC6YB3y/gkRsPCHe6ed
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

tIsPm6Ik8KaN1nrgS7ZklmOVhMJKzRwuJIczYOXD
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

iC3oheGe7IUIarFsNMkd7EgrO3jtZsSOeWmD3n+M
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

WGWVjUTR939+J399roD1B0y2PpxxVJkES/1Y+Zj0
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

3KjVSaLtx9zRSX8XAbjIho9OjIgrqJqpisXRAL34VOKa5Vt8sycX
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

6hN35N6MgSGtBxBHEa2HPQfRdbzP82Z+
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

88Tk5c0NqMuRkrF8hey1FGlmDoLnzc7ILaZRfyHBNVOFBkpdn627G190
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

+SvzZpA3
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

pRtK4dw4LTzcqx+QGtVKnO7RcGzM7vRX+Bi6hG6H
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

BOxonbRJRBD0ckscZOf85muQ3Wl9af0AVqW3rLatt8o+Ae+c
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

WyVgrtBIGu4G
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

Kc2Jo5YPSjXnTkLAdc0Hz+Ys63su
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

w5Khchf8R7agCyzxxN5DaAhqXzvwdmP7zAYspsbiDrW5viSP
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

qiAeW2GFZECpkJcNrVPSWh2HagCXZWK0vm9qp/UsQu0yrbYhnr68
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

m8rifOoCWCKR
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

eGADbkpyrqXRfboQnoZsG4q5WTP468SQvvG5
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

KuGUaJPHZnJED4AhMmwlxyOAgwrr
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

wpaYT05KnJe32x+SMsj/D1Fu1uwJ
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

0nxpbl/f5Wpl
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

i9fgojaGHAeCOnZT/cKi7e97sIBPWA9LUzm9
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

cu51OyLemGhmW/HGY0dVHLqlCFF1pkgl
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

XS/Dr55FBcOEArf9LAhST4Ldo/DUhgkC
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

UfUk7GRSJFClH9voy8l27OyCbvWFGFPouOOaKaqW04MjyaR7YbPMAuhd
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

NlK73nZfdklJrX+9ZSCyycErdhh2n1ax
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

UvToiIMrVCjU8jVStDKDYmlkDJGcn5fqdBb9HxEGmpv0
-----END CERTIFICATE-----`],qB=GB,hA=_,PB=OB,_B=()=>qB.rootCertificates||PB,uA=w=>!!_B().find(I=>{try{const C=hA.pki.certificateFromPem(I);return hA.pki.certificateToPem(w)===I||C.issued(w)}catch{return!1}}),jA=w=>!!w.find((I,C)=>w[C+1]&&w[C+1].issued(I)),$B=w=>!!w.find(({validity:{notAfter:I,notBefore:C}})=>I.getTime()<Date.now()||C.getTime()>Date.now()),An=w=>jA(w)&&uA(w[w.length-1]);var Bn={authenticateSignature:An,verifyCaBundle:jA,verifyRootCert:uA,isCertsExpired:$B};const nn=JB,En=KB,Mn=ZA,Qn=Bn;var yA={extractSignature:nn,...En,...Mn,...Qn};const In=_,{extractSignature:wn,getMessageFromSignature:Dn,preparePDF:gn}=yA,eA=w=>w.reduce((I,{name:C,value:V})=>(C&&(I[C]=V),I),{}),Cn=w=>{const{issuer:I,subject:C,validity:V}=w;return{issuedBy:eA(I.attributes),issuedTo:eA(C.attributes),validityPeriod:V,pemCertificate:In.pki.certificateToPem(w)}},zA=w=>w.map(Cn).map((I,C)=>C?I:{clientCertificate:!0,...I}),Nn=w=>{const I=gn(w),{signatureStr:C}=wn(I);return C.map(V=>{const{certificates:T}=Dn(V);return zA(T)})};var mA={extractCertificatesDetails:zA,getCertificatesInfoFromPDF:Nn};const x=_,aA=wA,{extractSignature:Gn,getMessageFromSignature:Vn,getClientCertificate:Tn,checkForSubFilter:Fn,preparePDF:rn,authenticateSignature:tn,sortCertificateChain:Un,isCertsExpired:cn}=yA,{extractCertificatesDetails:Yn}=mA,Rn=(w,I,C)=>{const V=Vn(w),{certificates:T,rawCapture:{signature:G,authenticatedAttributes:N,digestAlgorithm:Q}}=V,t=x.asn1.derToOid(Q),l=x.pki.oids[t].toLowerCase(),F=x.asn1.create(x.asn1.Class.UNIVERSAL,x.asn1.Type.SET,!0,N),i=Tn(T),o=x.md[l].create().update(x.asn1.toDer(F).data).digest().getBytes();if(!i.publicKey.verify(o,G))throw new aA("Wrong authenticated attributes",aA.VERIFY_SIGNATURE);const j=x.pki.oids.messageDigest,S=N.find(AA=>x.asn1.derToOid(AA.value[0].value)===j).value[1].value[0].value,k=x.md[l].create().update(I.toString("latin1")).digest().getBytes()===S,X=Un(T),Z=Yn(X),v=tn(X),f=cn(X);return{verified:k&&v&&!f,authenticity:v,integrity:k,expired:f,meta:{certs:Z,signatureMeta:C}}};var ln=w=>{const I=rn(w);Fn(I);try{const{signatureStr:C,signedData:V,signatureMeta:T}=Gn(I),G=V.map((N,Q)=>Rn(C[Q],N,T[Q]));return{verified:G.every(N=>N.verified===!0),authenticity:G.every(N=>N.authenticity===!0),integrity:G.every(N=>N.integrity===!0),expired:G.some(N=>N.expired===!0),signatures:G}}catch(C){return{verified:!1,message:C.message,error:C}}};const HA=ln,{getCertificatesInfoFromPDF:hn}=mA;Object.assign(HA,{getCertificatesInfoFromPDF:hn});var en=HA;const an=MB(en),on={style:{flex:"0 0 100%"}},dn={"mx-auto":"","max-w-600px":""},xn={"font-bold":""},bn={key:1},Zn={key:0,style:{flex:"0 0 100%"},"mt-5":"",flex:"","flex-col":"","gap-4":""},sn={"mb-2":"","font-bold":""},Jn=oA({__name:"pdf-signature-checker",setup(w){const I=EA([]),C=EA("idle"),V=EA(null);async function T(G){V.value=G;const N=await G.arrayBuffer();C.value="loading";try{const{signatures:Q}=an(N);I.value=Q,C.value="parsed"}catch{I.value=[],C.value="error"}}return(G,N)=>{const Q=DB,t=gB,l=CB,F=rB;return z(),J(YA,null,[W("div",on,[W("div",dn,[y(Q,{title:"Drag and drop a PDF file here, or click to select a file",accept:".pdf",onFileUpload:T}),u(V)?(z(),QB(t,{key:0,"mt-4":"",flex:"","gap-2":""},{default:m(()=>[W("div",xn,P(u(V).name),1),W("div",null,P(u(VB)(u(V).size)),1)]),_:1})):MA("",!0),u(C)==="error"?(z(),J("div",bn,[y(l,{"mt-4":""},{default:m(()=>[IB(" No signatures found in the provided file. ")]),_:1})])):MA("",!0)])]),u(C)==="parsed"&&u(I).length?(z(),J("div",Zn,[(z(!0),J(YA,null,wB(u(I),(i,o)=>(z(),J("div",{key:o},[W("div",sn," Signature "+P(o+1)+" certificates : ",1),y(F,{signature:i},null,8,["signature"])]))),128))])):MA("",!0)],64)}}});export{Jn as default};
